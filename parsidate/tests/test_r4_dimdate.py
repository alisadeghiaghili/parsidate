...
def dim_jan_1403():
    """First fortnight of 1403 — fast enough for CI."""
...
@pytest.mark.slow
def test_full_year_row_count() -> None:
...
def test_nowruz_flags(dim_jan_1403) -> None:
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030101].iloc[0]
    assert bool(row["is_holiday"]) is True
    assert bool(row["is_business_day"]) is False
    assert row["holiday_name"]


def test_first_business_after_nowruz(dim_jan_1403) -> None:
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030105].iloc[0]
    assert bool(row["is_business_day"]) is True


def test_friday_weekend(dim_jan_1403) -> None:
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030103].iloc[0]
    assert int(row["weekday"]) == 6
    assert bool(row["is_weekend"]) is True


def test_iso_columns(dim_jan_1403) -> None:
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030101].iloc[0]
...
def test_month_bounds(dim_jan_1403) -> None:
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030115].iloc[0]
    assert str(row["month_start"]).endswith("01")
    assert str(row["month_end"]).endswith("31")  # Farvardin has 31 days


def test_fiscal_columns(dim_jan_1403) -> None:
    assert "fiscal_year" in dim_jan_1403.columns
    assert "fiscal_quarter" in dim_jan_1403.columns
    assert "fiscal_month" in dim_jan_1403.columns
    row = dim_jan_1403[dim_jan_1403["date_key"] == 14030101].iloc[0]
    assert int(row["fiscal_month"]) == 1


def test_ytd_mtd_flags() -> None:
...
def test_column_dictionary_stable(dim_jan_1403) -> None:
    required = {
    ...
    assert required.issubset(set(dim_jan_1403.columns))
